...
| Current runtime images and demo screenshots | Project-authored CD symbol: GPL-3.0-or-later. Historical bird artwork retains its applicable terms; no blanket relicensing | Owner approved restoring the 26 PNG assets after the artwork research, then supplied songbirdimage.png as the missing-artwork replacement. Current hashes and the recorded selections are in [artwork decision](../publication/artwork-decision.json). The README reuses demo captures listed in [screenshot notes](screenshots/README.md). |
